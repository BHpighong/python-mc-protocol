...
class McProtocolRW:
    ...
    def point_status_qty (self, point_status):
        ...
        qty_in_ascii = self.str_to_ascii(len(point_status), 4)
    ...
    def data_qty_unit (self, words):
        ...
        point_qty_ascii = self.str_to_ascii(len(words), 4)
        data_ascii = ' '.join([self.str_to_ascii(f"{w:X}"[2:], 4) for w in words])
    ...
    def build_command_payload(self, headdevice, values, command_code):
        ...
        if dev_type == 'M':
            data_payload = self.point_status_qty(values)
        elif dev_type == 'D':
            data_payload = self.data_qty_unit(values)
        else:
    ...
    def build_command_payload_by_values(self, headdevice, values, command_code):
        """ pay load building """
        dev_type = headdevice[0]
        device_code = self.device_code_define(dev_type)
        device_address = self.str_to_ascii(headdevice[1:], 6)
        ...
        if dev_type == 'M':
            data_payload = self.str_to_ascii(values, 4) #self.point_status_qty(values)
        elif dev_type == 'D':
            data_payload = self.str_to_ascii(values, 4) #self.data_qty_unit(values)
        else:
            raise ValueError("Unsupported device type")
    ...
    def build_random_write_payload(self, headdevices, values, is_bit=True):
        ...
        if is_bit:
            ...
            qty = self.str_to_ascii(len(headdevices), 2)
            word_pack = ''
            ...
            qty_dwords = self.str_to_ascii(0, 2)
            word_pack = ''
            for i, device in enumerate(headdevices):
                device_code = self.device_code_define(device[0])
                device_address = self.str_to_ascii(device[1:], 6)
