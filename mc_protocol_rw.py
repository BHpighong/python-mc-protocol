...
class McProtocolRW:
    ...
    def point_status_qty (self, point_status):
        ...
        #point_status = [0, 1] #debug測試點
        point_qty_int = len(point_status)
        point_qty_in_hex = self.str_to_ascii(point_qty_int, 4)

        #list transfor from int to Ascii
        point_status_ascii = [1]*point_qty_int #避免錯誤定預設1個，status in '0'

        for i in range (point_qty_int):
            point_status_ascii[i] = format(ord(str(point_status[i])), 'X')

        point_in_str = ' '

        return point_qty_in_hex+' '+point_in_str.join(point_status_ascii)
    ...
    def command_random (self, headdevice, values):
        """ Random_write """
        i = 0
        command_code = None
        ...
        if headdevice[0][0] == 'M': #目前針對M機碼
            ...
            command_code = self.enq_code+' '+code_crc+' '+crc_code+' '+self.end_code
        ...
        return bytes.fromhex( command_code )
